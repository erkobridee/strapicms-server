...
- This plugin is a WYSIWYG editor for Strapi v4. It uses the `@mdxeditor/editor` to provide a rich text editor for Strapi.

- Allows you to replace default Strapi v4 WYSIWYG editor with a MdxEditor.
...
const customComponents = {
  a: ({ node, className, ...props }) => {
    ...
    return <Link className={cn(className, "link")} target={isExternalLink ? "_blank" : "_self"} rel={isExternalLink ? "noopener nofollow" : "follow"} {...props} />;
  },
